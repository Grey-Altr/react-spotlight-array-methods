...
export const filterOceania = (countries) => {
  return countries.filter((oceaniaCountry) => oceaniaCountry.continent === 'Oceania');
};
...
export const filterYoungDogs = (dogs) => {
  return dogs.filter((youngDog) => youngDog.age <= 2);
};
...
export const getYoungDogNames = (dogs) => {
  return dogs.filter((youngDog) => youngDog.age <= 2).map((youngDog) => youngDog.name);
};
